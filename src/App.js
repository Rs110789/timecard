            ...
            {employees.map(name => {
              ...
              return (
                <div key={name} style={{background:done?"#c8e6c9":working?"#fff9c4":"#fff",borderRadius:"12px",padding:"14px 12px",boxShadow:"0 1px 6px rgba(0,0,0,0.08)",border:done?"1px solid #a5d6a7":working?"1px solid #fff176":"1px solid #e8f5e9",transition:"all 0.2s"}}>
                  ...
                  {rec.in && !rec.out && <button onClick={async () => { const time = nowTime(); const res = await fetch(`${SUPABASE_URL}/rest/v1/timecard_records?employee_name=eq.${encodeURIComponent(name)}&date=eq.${encodeURIComponent(todayStr())}`, { method: "PATCH", headers: { "apikey": SUPABASE_KEY, "Authorization": `Bearer ${SUPABASE_KEY}`, "Content-Type": "application/json", "Prefer": "return=minimal" }, body: JSON.stringify({ time_out: time }) }); if (!res.ok) { showToast("失敗: " + await res.text(), "#e57373"); return; } setRecords(prev => ({ ...prev, [todayStr()]: { ...(prev[todayStr()]||{}), [name]: { ...getRecord(todayStr(), name), out: time } } })); showToast(`${name} の退勤 (${time})`, "#e57373"); }} style={{width:"100%",padding:"9px",borderRadius:"8px",border:"none",background:"linear-gradient(135deg,#ef5350,#c62828)",color:"#fff",fontSize:"13px",fontWeight:"700",cursor:"pointer"}}>退勤</button>}
